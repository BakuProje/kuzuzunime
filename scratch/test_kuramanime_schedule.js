...
async function testSchedule() {
  try {
    const res = await axios.get('https://kuramanime.pro/schedule', {
      httpsAgent: dohAgent,
      headers,
      timeout: 8000
    });
    const $ = cheerio.load(res.data);
    console.log('Schedule Page Title:', $('title').text());
    
    // Look for day tabs or containers
    console.log('Schedule cards count:', $('.product__item, .anime__item').length);
    console.log('Schedule day tabs count:', $('.nav-tabs li, .schedule-tabs li, [data-day], .col-lg-12').length);

    $('.product__item, .anime__item').slice(0, 5).each((i, el) => {
      const title = $(el).find('h5, h6, a').first().text().trim().replace(/\s+/g, ' ');
      const href = $(el).find('a').attr('href');
      const img = $(el).find('[data-setbg]').attr('data-setbg') || $(el).find('img').attr('src');
      console.log(`Sched ${i}: title="${title}" href="${href}" img="${img}"`);
    });
  } catch (e) {
    console.error('Schedule test error:', e.message);
  }
}
