...
async function inspectScriptSrcs() {
  ...
  console.log('=== All Script tags with src ===');
  $('script[src]').each((i, el) => {
    console.log(`Script ${i}: ${$(el).attr('src')}`);
  });

  console.log('\n=== Download buttons or links ===');
  $('a[href*="download"], a[href*="drive"], a[href*="mega"], a[href*="file"], .anime__details__btn a').each((i, el) => {
    console.log(`Dl link ${i}: text="${$(el).text().trim()}" href="${$(el).attr('href')}"`);
  });
}
