...
async function getKitsuGenreItems(genre) {
  try {
...
async function test() {
  const genres = ['Action', 'Romance', 'Isekai', 'Sci-Fi', 'Fantasy', 'Horror', 'Mecha', 'Comedy'];
  for (const g of genres) {
    const list = await getKitsuGenreItems(g);
    console.log(`[Genre: ${g}] Result count: ${list.length}, 1st item: ${list[0]?.title}, image: ${list[0]?.image?.slice(0, 40)}...`);
  }
}
