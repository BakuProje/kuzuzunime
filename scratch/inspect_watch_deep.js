...
async function inspectWatchPlayerDeep() {
  ...
  console.log('=== #player Section Full HTML ===');
  console.log($('#player').html());

  console.log('=== #changeServer Options ===');
  $('#changeServer option').each((i, el) => {
    console.log(`Option ${i}: text="${$(el).text().trim()}" value="${$(el).attr('value')}" data-src="${$(el).attr('data-src')}"`);
  });

  console.log('=== All Inline Scripts ===');
