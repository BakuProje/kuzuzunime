...
async function inspectLeviathan() {
  try {
    const res = await axios.get('https://v20.kuramanime.ing/storage/leviathan.js?v=1448', {
      httpsAgent: dohAgent,
      headers,
      timeout: 8000
    });
    console.log('Leviathan JS content length:', res.data.length);
    console.log('Leviathan excerpt:\n', res.data.slice(0, 1000));
  } catch (e) {
    console.error('Leviathan error:', e.message);
  }
}
