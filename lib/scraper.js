...
export async function detail(link) {
  try {
    ...
    const response = await axios.get(targetUrl, { headers, timeout: 6000 });
    const $ = cheerio.load(response.data);
  ...
  try {
    const relative = getRelativeUrl(link);
    const res = await axios.get(`https://puruboy-api.vercel.app/api/anime/samehadaku/detail?url=${encodeURIComponent(relative)}`, { headers, timeout: 6000 });
    if (res.data && res.data.success && res.data.result) {
...
export async function download(link) {
  try {
    const relative = getRelativeUrl(link);
    const res = await axios.get(`https://puruboy-api.vercel.app/api/anime/samehadaku/stream?url=${encodeURIComponent(relative)}`, { headers, timeout: 6000 });
    if (res.data && res.data.success && res.data.result) {
      const result = res.data.result;
