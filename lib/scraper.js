...
export async function download(link) {
      ...
      try {
        ...
        $s('a[href*="/anime/"]').each((i, el) => {
          ...
          if (h && !animeLinks.some(x => x.href === h)) {
            ...
            const sim = getSimilarity(cleanName, t);
            if (sim >= 0.35 || t.toLowerCase().includes(cleanName.toLowerCase()) || cleanName.toLowerCase().includes(t.toLowerCase())) {
  ...
  // Step 3: Combine Streams according to Anime type (Music vs TV Series)
  const combinedStreams = [...foundNativeStreams];

  if (isMusic) {
    // Top-tier Audio & MV players for music tracks
    const musicStreams = [
      {
        server: 'Music Audio & MV (Official)',
        url: `https://www.youtube-nocookie.com/embed?listType=search&list=${encodeURIComponent(cleanName + ' official music audio animation')}`
      },
      {
        server: 'Music Player HD (Studio)',
        url: `https://www.youtube-nocookie.com/embed?listType=search&list=${encodeURIComponent(cleanName + ' theme song mv')}`
      },
      {
    ...
    for (const ms of musicStreams) {
      if (!combinedStreams.some(s => s.url === ms.url)) {
        combinedStreams.push(ms);
      }
    }
  } else {
    // Multi-server fast streaming fallbacks for TV Series / Movies
    const universalStreams = [
      {
        server: 'Server HD 1 (MultiEmbed)',
        url: `https://multiembed.mov/directstream.php?video_id=${slugifiedTitle}&s=1&e=${epNum}`
      },
      {
        server: 'Server HD 2 (VidLink Pro)',
        url: `https://vidlink.pro/anime/${slugifiedTitle}/${epNum}`
      },
      {
        server: 'Server HD 3 (SmashyStream)',
        url: `https://player.smashy.stream/anime/${slugifiedTitle}?e=${epNum}`
      },
      {
        server: 'Server HD 4 (Vidsrc VIP)',
        url: `https://vidsrc.pm/embed/anime/${slugifiedTitle}/${epNum}`
      },
      {
        server: 'Server HD 5 (Trailer & Theme MV)',
        url: `https://www.youtube-nocookie.com/embed?listType=search&list=${encodeURIComponent(cleanName + ` episode ${epNum} official anime`)}`
      }
    ];
