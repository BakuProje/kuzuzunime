...
async function axiosGetFast(url, config = {}, timeoutMs = 6000) {
  let proxyBase = process.env.SCRAPER_PROXY_URL || process.env.NEXT_PUBLIC_SCRAPER_PROXY_URL;
  // If no proxy is set, and it's running on Vercel, it might hit Cloudflare blocks. 
  // Edge runtime usually bypasses it, but we can also use our own edge proxy as a fallback.
  // Actually, let's just rely on Edge runtime for now.
...
// Dual Engine Scraper: Kitsu.app + Jikan.moe (MyAnimeList)
export async function getAnimePoster(title, fallbackImage = '') {
  ...
  // 2. Secondary Engine: Jikan.moe (MyAnimeList v4)
  for (const q of uniqueQueries.slice(0, 2)) {
    try {
      const jikanRes = await axiosGetFast(`https://api.jikan.moe/v4/anime?q=${encodeURIComponent(q)}&limit=1`, {
        headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/130.0.0.0 Safari/537.36' }
      }, 3000);
      const jItem = jikanRes.data?.data?.[0];
      const jPoster = jItem?.images?.webp?.large_image_url || jItem?.images?.jpg?.large_image_url;
      if (jPoster) {
        posterCache.set(cacheKey, jPoster);
        return jPoster;
      }
    } catch (e) {}
  }
...
export async function getAniListData(title) {
  ...
  try {
    ...
    if (anime) {
      const result = {
        ...
        romajiTitle: anime.titles?.en_jp || anime.canonicalTitle,
        ...
        poster: anime.posterImage?.large || anime.posterImage?.original || anime.posterImage?.medium,
  ...
  // 2. Jikan.moe Engine (MyAnimeList API)
  try {
    const jikanRes = await axiosGetFast(`https://api.jikan.moe/v4/anime?q=${encodeURIComponent(title)}&limit=1`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/130.0.0.0 Safari/537.36' }
    }, 3000);
    const item = jikanRes.data?.data?.[0];
    if (item) {
      const result = {
        ...
        studio: item.studios?.[0]?.name || 'Zunime'
      };
      posterCache.set(`anilist_${cacheKey}`, result);
      return result;
    }
