...
const dnsCache = new Map([
  ...
  ['otakudesu.best', '172.67.174.36'],
  ['otakudesu.cloud', '172.67.160.128'],
...
export async function download(link) {
  ...
  const extractIframesFromHtml = (html) => {
    ...
    // 1. Extract from standard <iframe> elements
    $('iframe').each((i, el) => {
      ...
      if (src) {
        ...
        else if (src.includes('desu') || src.includes('otakudesu')) serverName = `DesuStream HD ${streams.length + 1}`;
    ...
    $('select option, .mirror option, #server option').each((i, el) => {
      ...
      if (val) {
        ...
        if (val.startsWith('http') && (val.includes('putarin') || val.includes('mogo') || val.includes('desu') || val.includes('embed') || val.includes('player'))) {
          let serverName = text || `Server HD ${streams.length + 1}`;
          if (val.includes('putarin')) serverName = `Putarin HD ${streams.length + 1}`;
          else if (val.includes('mogo')) serverName = `Mogo HD ${streams.length + 1}`;
          else if (val.includes('desu')) serverName = `DesuStream HD ${streams.length + 1}`;
          
          if (!streams.some(s => s.url === val)) {
            streams.push({ server: serverName, url: val });
          }
        }
      }
    });
  ...
  // Step 3: Combine Native Putarin/Mogo Streams with Verified MAL ID-based HD Embeds
  const combinedStreams = [...foundNativeStreams];

  try {
    const malId = await getAnimeMalId(cleanName);
    if (malId) {
      const universalMalStreams = [
        {
          server: 'Server HD (VidLink VIP)',
          url: `https://vidlink.pro/anime/${malId}/${epNum}`
        },
        {
          server: 'Server HD (VidSrc VIP)',
          url: `https://vidsrc.me/embed/anime/${malId}/${epNum}`
        },
        {
          server: 'Server HD (AutoEmbed)',
          url: `https://player.autoembed.cc/embed/anime/${malId}/${epNum}`
        }
      ];

      for (const us of universalMalStreams) {
