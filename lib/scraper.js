...
export async function download(link) {
  ...
  // Fast scraper attempt with 2.2s hard deadline
  const scrapePromise = (async () => {
    // Attempt 1: Direct Samehadaku match
    try {
      ...
      if (res && res.data) {
        ...
        if (streams.length > 0) {
          const $ = cheerio.load(res.data);
          const title = $('title').text().split(' Subtitle ')[0].replace(/ – Samehadaku$/i, '').trim() || safeTitle;
          return { title, streams };
        }
      }
    } catch (e) {}

    // Attempt 2: Search top query on Samehadaku
    try {
      const searchRes = await fetchSamehadakuWithMirror(`?s=${encodeURIComponent(cleanName)}`);
      if (searchRes && searchRes.data) {
        const $s = cheerio.load(searchRes.data);
        let firstAnimeHref = null;
        $s('article a, .animpost a, a[href*="/anime/"]').each((i, el) => {
          if (firstAnimeHref) return;
          const h = $s(el).attr('href');
          if (h && h.includes('/anime/')) firstAnimeHref = h;
        });

        if (firstAnimeHref) {
          const aPath = firstAnimeHref.startsWith('http') ? new URL(firstAnimeHref).pathname.replace(/^\//, '') : firstAnimeHref.replace(/^\//, '');
          ...
          if (detailRes && detailRes.data) {
            ...
            $d('a[href*="/nonton/"], a[href*="/episode/"]').each((i, el) => {
              ...
              if (epRegex.test(text) || href.toLowerCase().includes(`episode-${epNum}-`)) {
            ...
            if (matchedNontonUrl) {
              ...
              if (epRes && epRes.data) {
                ...
                if (streams.length > 0) {
                  ...
                  const title = $w('title').text().split(' Subtitle ')[0].replace(/ – Samehadaku$/i, '').trim() || safeTitle;
                  return { title, streams };
                }
              }
            }
  ...
  // Verified Active Universal Embed Fallbacks (Multi-Server)
  return {
    title: safeTitle,
    streams: [
      {
