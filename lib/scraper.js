...
// 5. DOWNLOAD / WATCH - SMART SAMEHADAKU STREAM RESOLVER
export async function download(link) {
  ...
  const extractIframesFromHtml = (html) => {
    ...
    $('iframe').each((i, el) => {
      ...
      if (src) {
        ...
        let serverName = `Samehadaku HD ${streams.length + 1}`;
  ...
  // Step 2: Search Samehadaku with intelligent keyword fallback
  ...
  for (const q of uniqueQueries) {
    try {
      ...
      for (const animeHref of animeLinks.slice(0, 2)) {
