...
// 1. ANIMETERBARU (LATEST) - KURAMANIME WITH SAMEHADAKU & KITSU FALLBACK
...
// 2. POPULER - KURAMANIME WITH SAMEHADAKU & KITSU FALLBACK
...
// 3. SEARCH - KURAMANIME + SAMEHADAKU + KITSU
...
export async function detail(link) {
  ...
  if (htmlData) {
    try {
      ...
      let image = $('.sh-det-po img, .sh-kolom img, .anime__details__pic, [data-setbg]').first().attr('src') || $('.anime__details__pic, [data-setbg]').attr('data-setbg') || '';
      if (image.includes('placeholder.svg')) image = '';
...
// 5. DOWNLOAD / WATCH - SMART MULTI-SOURCE RESOLVER
export async function download(link) {
  ...
  // Step 1: Direct path fetch on Samehadaku mirrors
  try {
    const targetPath = `nonton/${rawClean}/`;
    const res = await fetchSamehadakuWithMirror(targetPath);
    if (res && res.data) {
      const $ = cheerio.load(res.data);
      const title = $('title').text().split(' Subtitle ')[0].replace(/ – Samehadaku$/i, '').trim() || rawClean.replace(/-/g, ' ');
  ...
  const cleanName = rawClean
    .replace(/^(?:\d+\/)/, '') // strip leading kuramanime id e.g. 2475/
    .replace(/(?:-episode-|-eps-|-ep-|\/episode\/)\d+.*$/i, '')
    .replace(/-sub-indo.*$/i, '')
    .replace(/-/g, ' ')
    .trim();

  // Generate intelligent search variants:
  ...
  if (words.length > 4) {
    searchKeywords.push(words.slice(0, 4).join(' '));
    searchKeywords.push(words.slice(0, 3).join(' '));
  } else if (words.length > 2) {
    searchKeywords.push(words.slice(0, 2).join(' '));
  }

  // Fetch title synonyms from Kitsu (e.g. Solo Leveling -> Ore dake Level Up na Ken)
  try {
    const kitsuRes = await axiosGetFast(`https://kitsu.io/api/edge/anime?filter[text]=${encodeURIComponent(cleanName)}&page[limit]=1`, {
      headers: { 'Accept': 'application/vnd.api+json' }
    }, 2500);
    const attr = kitsuRes.data?.data?.[0]?.attributes;
    if (attr) {
      if (attr.canonicalTitle) searchKeywords.push(attr.canonicalTitle);
      if (attr.titles?.en) searchKeywords.push(attr.titles.en);
      if (attr.titles?.en_jp) searchKeywords.push(attr.titles.en_jp);
      if (attr.abbreviatedTitles && Array.isArray(attr.abbreviatedTitles)) {
        searchKeywords.push(...attr.abbreviatedTitles);
      }
    }
  } catch (kitsuErr) {}

  const uniqueKeywords = [...new Set(searchKeywords.filter(q => q && q.length >= 2))];

  for (const q of uniqueKeywords) {
    try {
      ...
      for (const animeHref of animeHrefs.slice(0, 3)) {
        ...
        $d('a[href*="/nonton/"], a[href*="/episode/"]').each((i, el) => {
        ...
        if (matchedNontonUrl) {
          const epPath = matchedNontonUrl.startsWith('http') ? new URL(matchedNontonUrl).pathname.replace(/^\//, '') : matchedNontonUrl.replace(/^\//, '');
  ...
  // Step 3: Verified Active Universal Embed Fallbacks (Multi-Server)
    ...
    streams: [
      {
        ...
        url: `https://vidsrc.pm/embed/anime/${slugifiedTitle}/${epNum}`
      },
      {
...
// 6. SCHEDULE - KURAMANIME WITH SAMEHADAKU & KITSU FALLBACK
