...
export async function animeterbaru(page = 1) {
  ...
  try {
    const kitsuRes = await axiosGetFast('https://kitsu.io/api/edge/anime?sort=-createdAt&page[limit]=28', {
      headers: { 'Accept': 'application/vnd.api+json' }
    }, 3500);
    ...
    return items.map((item, idx) => {
      ...
      return {
        ...
        episode: item.attributes?.episodeCount ? `Episode ${item.attributes.episodeCount}` : 'Episode Terbaru',
...
export async function schedule() {
  ...
  try {
    const kitsuRes = await axiosGetFast('https://kitsu.io/api/edge/trending/anime?limit=28', {
      headers: { 'Accept': 'application/vnd.api+json' }
    }, 3500);
    const items = kitsuRes.data?.data || [];
    return items.map(item => ({
