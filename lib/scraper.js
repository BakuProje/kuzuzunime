...
export async function download(link) {
  ...
  // Helper to extract iframes and video players from HTML
  ...
  const epMatch = watchSlug.match(/(?:episode|eps|ep)[-_ ]*(\d+(\.\d+)?)/i);
  const epNum = epMatch ? epMatch[1] : '1';
  const cleanName = watchSlug
    .replace(/(?:-episode-|-eps-|-ep-)\d+.*$/i, '')
    .replace(/-sub-indo.*$/i, '')
    .replace(/-/g, ' ')
    .trim();

  // Helper to fetch Japanese Romaji aliases (e.g. Solo Leveling -> Ore dake Level Up na Ken)
  async function getTitleAliases(name) {
    try {
      const res = await axiosGetFast(`https://kitsu.io/api/edge/anime?filter[text]=${encodeURIComponent(name)}&page[limit]=1`, {
        headers: { 'Accept': 'application/vnd.api+json' }
      }, 2000);
  ...
  const aliases = await getTitleAliases(cleanName);
